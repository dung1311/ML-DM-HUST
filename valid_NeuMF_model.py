...
def plot_grid_search_results(results):
    ...
    param_names = ['num_factors', 'lr', 'weight_decay', 'nums_hiddens', 'epoch']
...
def grid_search_neumf(param_grid, df, num_users, num_items, devices):
    ...
    for factors, lr, wd, hiddens, epochs in product(param_grid['num_factors'], 
                                                    param_grid['lr'],
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(df)):
            ...
            train_iter = DataLoader(train_ds, batch_size=1024, shuffle=True)
            val_iter = DataLoader(val_ds, batch_size=1024)

            model = NeuMF_model.NeuMF(factors, num_users, num_items, hiddens).to(devices)
        ...
        print(f"Tested params: factors={factors}, lr={lr}, wd={wd}, hiddens={hiddens}, epoch={epochs} → RMSE: {avg_rmse:.4f}")
