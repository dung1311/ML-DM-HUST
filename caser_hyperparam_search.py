...
def evaluate(model, data_loader, device):
    ...
    with torch.no_grad():
        for user, seq, rating in tqdm(data_loader, desc=f"Epoch {epoch+1}"):
...
def train(model, train_loader, optimizer, device, epochs=5):
    ...
    for epoch in range(epochs):
        for user, seq, rating in train_loader:
            user, seq, rating = user.to(device), seq.to(device), rating.to(device)
            pred = model(user, seq)
            loss = criterion(pred, rating.squeeze())
...
if __name__ == "__main__":
    ...
    for num_factors, L, d, d_prime, drop_ratio, lr, batch_size, wd, epoch in all_combinations:
        print(f"Trying: num_factors={num_factors}, L={L}, d={d}, d_prime={d_prime}, drop_ratio={drop_ratio}, lr={lr}, batch_size={batch_size}, weight_decay={wd}, epoch={epoch}")
