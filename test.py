...
class ContentFiltering:
    def __init__(self, users, items, ratings_base, ratings_test):
        ...
        self.models = {}
        self.selected_features = self.tfidf

    def feature_selection(self, method: Literal['chi2', 'mutual_info', 'pca', 'lasso', 'rf'], n_features: int = 10):
        if method == 'chi2':
            item_indices = self.ratings_base['item_id'].values - 1
            ratings = self.ratings_base['rating'].values
            X = self.tfidf[item_indices]
            selector = SelectKBest(chi2, k=n_features)
            self.selected_features = selector.fit_transform(X, ratings)
            ...
            self.selected_features = pca.fit_transform(self.tfidf)

        elif method == 'lasso':
            lasso = Lasso(alpha=0.01, max_iter=5000)
            lasso.fit(self.tfidf, self.ratings_base['rating'])
            importance = np.abs(lasso.coef_)
            top_features = np.argsort(importance)[-n_features:]
            self.selected_features = self.tfidf[:, top_features]

        elif method == 'rf':
            rf = RandomForestRegressor(n_estimators=100, random_state=42)
            rf.fit(self.tfidf, self.ratings_base['rating'])
            importance = rf.feature_importances_
            top_features = np.argsort(importance)[-n_features:]
            self.selected_features = self.tfidf[:, top_features]

        else:
            raise ValueError("Invalid method. Choose from 'chi2', 'mutual_info', 'pca', 'lasso', 'rf'.")
    ...
    def train(self, algo: Literal['ridge', 'lasso'] = 'ridge', alpha=1.0):
        for uid in self.all_users:
            ...
            X = self.selected_features[item_ids - 1]
        ...
        for user_index, uid in enumerate(self.all_users):
            if uid in self.models:
                ...
                predict_matrix[user_index] = model.predict(self.selected_features)
    ...
    def predict_rating(self, user_id, item_id):
        if user_id in self.models:
            ...
            item_vector = self.selected_features[item_id - 1].reshape(1, -1)
...
if __name__ == "__main__":
    ...
    cf.feature_selection(method='chi2', n_features=10)
    cf.train(algo='ridge', alpha=0.5)
